...
import {
	C, N, F, GenLex, Streams, type SingleParser, type Response as MasalaResponse, type Tuple, type Option,
} from '@domdomegg/masala-parser';
...
class TypeFuncApp {
	...
	constructor(readonly constructorName: TypeFunc, ...args: MonoType[]) {
		this.args = args;
	}
...
type Context = Record<string, PolyType | undefined>;
type Substitution = Record<string, MonoType | undefined>;
...
const assertIsTypeInferenceError = (e: unknown): asserts e is TypeInferenceError => {
	if (!isTypeInferenceError(e)) {
		throw new Error(`Expected TypeInferenceError but got ${String(e)}`);
	}
};
...
function combine(...substitutions: Substitution[]): Substitution {
	...
	if (substitutions.length > 2) {
		return combine(substitutions[0], combine(...substitutions.slice(1)));
	}
...
const toTuple = (tuple: Tuple<unknown | number | Expr[]>): Expr => {
	...
	const result = nestLeft([new Var(','.repeat(elements.length - 1), {start, end: start + 1}), ...elements]);
...
function parse(code: string, forResponse = false): Expr | Response<Expr> {
...
const typeUtils = {
	number, char, boolean, f, list, tuple, maybe, either, a, b, c, d, pt, standardCtx,
};
export {
	CharLiteral, NumberLiteral, Var, App, Abs, Let, type Expr,
	type MonoType, TypeVar, type TypeFunc, TypeFuncApp, PolyType, type Context, type Substitution,
	ParseError, TypeInferenceError, isTypeInferenceError, assertIsTypeInferenceError,
	type Response, type Rejected, type Accepted, type TypeResult,
	typeUtils,
