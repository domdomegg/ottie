import {
	TypeVar, TypeFuncApp, type MonoType, PolyType, type Context, type Expr, Var, App, Abs, Let, CharLiteral, NumberLiteral, typeUtils, type Response, type TypeResult, TypeInferenceError, isTypeInferenceError, assertIsTypeInferenceError, type Substitution, inst, unify, apply, combine, generalise,
} from 'language';
...
function _infer(expr: Expr, ctx: Context, type: MonoType, freshTypeName: () => string, logger: (message: string, notes: Map<Expr, string>) => void = () => {/* noop */}): Substitution {
	if (expr instanceof CharLiteral) {
