import {
	TypeVar, TypeFuncApp, type MonoType, PolyType, type Context, type Expr, Var, App, Abs, Let, CharLiteral, NumberLiteral, typeUtils, type Response, type TypeResult, TypeInferenceError, isTypeInferenceError, assertIsTypeInferenceError, type Substitution, inst, unify, apply, combine, generalise, freeVars,
} from 'language';
...
function unifySubtitutions(s0: Substitution, s1: Substitution): Substitution {
	...
	const t0 = Object.entries(s0).filter(([k]) => !(k in s1));
	const t1 = Object.entries(s1).filter(([k]) => !(k in s0));

	const s = t0.reduce<Substitution>((acc, cur) => {
		acc[cur[0]] = cur[1];
		return acc;
	}, {});
	for (const [alpha, tau] of t1) {
	...
	let v = s;
	for (const d of dn) {
		const beta = new TypeVar(d);
		const tau0 = apply(v, apply(s0, beta));
		const tau1 = apply(v, apply(s1, beta));
		if (freeVars(tau0).includes(d)) {
			throw new TypeInferenceError(`Occurs check failed. \`${d.toString()}\` occurs in \`${tau0.toString()}\` so unifying them would create an infinite type.`);
		}

		if (freeVars(tau1).includes(d)) {
			throw new TypeInferenceError(`Occurs check failed. \`${d.toString()}\` occurs in \`${tau1.toString()}\` so unifying them would create an infinite type.`);
		}
...
function infer(expr: Expr, forResponse = false, ctx: Context = typeUtils.standardCtx): MonoType | Response<TypeResult, Omit<TypeResult, 'type'>> {
	let typeCounter = 0;
...
const cloneAst = (expr: Expr, notes: Map<Expr, string>): Expr => {
	if (expr instanceof CharLiteral) {
		return new CharLiteral(expr.value, expr.pos, notes.get(expr));
	}

	if (expr instanceof NumberLiteral) {
		return new NumberLiteral(expr.value, expr.pos, notes.get(expr));
	}

	if (expr instanceof Var) {
		return new Var(expr.name, expr.pos, notes.get(expr));
	}

	if (expr instanceof App) {
		return new App(cloneAst(expr.func, notes), cloneAst(expr.arg, notes), expr.pos, notes.get(expr));
	}

	if (expr instanceof Abs) {
		return new Abs(expr.param, cloneAst(expr.body, notes), expr.pos, notes.get(expr));
	}

	if (expr instanceof Let) {
		return new Let(expr.param, cloneAst(expr.def, notes), cloneAst(expr.body, notes), expr.pos, notes.get(expr));
	}
...
const highlight = (expr: Expr): Map<Expr, string> => {
	const notes = new Map<Expr, string>();
...
function _infer(expr: Expr, ctx: Context, freshTypeName: () => string, logger: (message: string, notes: Map<Expr, string>) => void = () => {/* noop */}): [MonoType, Substitution] {
	if (expr instanceof CharLiteral) {
